% Compare SC-simFC (50x50 matrix), (unbiased, because based on simFC's
% based on every subs 'ideal' csfs), average simFC-SC corrs across
...
for i = 1:length(subs) % index of SC. Loop thru SC, fix on one, go into its simFCs in following loop
	...
	for j=1:length(subs) %loop thru simFC?s, need to go through all params of each simFC too 
		...
		for k=1:length(ideal_csf) %looping thru the (50) csfs each time getting its simFC. 
			...
			ideal_csf_simempcorrs(end+1)=corr(simFC_reshaped, SC_reshaped);
		end	
