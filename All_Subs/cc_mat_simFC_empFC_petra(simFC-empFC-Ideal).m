% Compare SC-simFC (50*50 matrix), (unbiased, because based on 101 simFC's (1 for each csf)) and max of simFC-SC corrs across all simFCs
%
% cc_mat:
% 		SC (i)
% SimFC (j)
...
for i = 1:length(subs) % index of SC. Loop thru SC, fix on one, go into its simFCs in following loop
	...
	for j=1:length(subs) %loop thru simFCs, need to go through all params of each simFC too
		...
		for k=1:length(csf) %looping thru the (101) csfs each time getting its simFC.
			...
			csf_simempcorrs(end+1)=corr(simFC_reshaped, SC_reshaped); %will be 101 long at the end
		end	
...
ylabel('SCs Per Subject', 'fontsize', 14);
title('Corr between SC and SimFC (unbiased) across subjects', 'fontsize', 14);